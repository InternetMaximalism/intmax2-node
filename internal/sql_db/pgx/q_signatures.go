...
func (p *pgx) CreateSignature(signature string) (*mDBApp.Signature, error) {
	s := models.Signature{
	...
	const (
		q = ` INSERT INTO signatures
              (signature_id, signature, created_at)
              VALUES ($1, $2, $3) `
	)

	_, err := p.exec(p.ctx, q,
		s.SignatureID, s.Signature, s.CreatedAt)
	if err != nil {
...
func (p *pgx) SignatureByID(txID string) (*mDBApp.Signature, error) {
	const (
...
func (p *pgx) sToDBApp(s *models.Signature) mDBApp.Signature {
	...
	if s.ProposalBlockID.Valid {
		m.ProposalBlockID = s.ProposalBlockID.String
	}
