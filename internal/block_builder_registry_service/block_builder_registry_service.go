...
func (bbr *blockBuilderRegistryService) GetBlockBuilder(
	...
	w, err = mnemonic_wallet.New().WalletFromPrivateKeyHex(bbr.cfg.Blockchain.BuilderPrivateKeyHex)
	if err != nil {
		open_telemetry.MarkSpanError(spanCtx, err)
...
func (bbr *blockBuilderRegistryService) UpdateBlockBuilder(
	...
	privateKey, err := crypto.HexToECDSA(bbr.cfg.Blockchain.BuilderPrivateKeyHex)
	if err != nil {
		open_telemetry.MarkSpanError(spanCtx, err)
...
func createTransactor(cfg *configs.Config) (*bind.TransactOpts, error) {
	privateKey, err := crypto.HexToECDSA(cfg.Blockchain.BuilderPrivateKeyHex)
	if err != nil {
