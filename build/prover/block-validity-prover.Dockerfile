...
WORKDIR /app
ADD . .
RUN cargo build --release --features dummy_proof --bin block-validity-prover

FROM alpine:3.20 AS intmax2-rust-run-env
COPY --from=intmax2-rust-build-env /app/target/release/block-validity-prover /app/block-validity-prover
