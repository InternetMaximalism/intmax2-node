...
import (
	...
	getBackupTransactions "intmax2-node/internal/use_cases/get_backup_transactions_list"
...
func (s *StoreVaultServer) GetBackupTransactionsList(
	...
	input := getBackupTransactions.UCGetBackupTransactionsListInput{
		Sender:  req.Sender,
	...
	if req.Pagination != nil {
		input.Pagination = &getBackupTransactions.UCGetBackupTransactionsListPaginationInput{
			Direction: mFL.Direction(req.Pagination.Direction),
			PerPage:   req.Pagination.PerPage,
		}

		if req.Pagination.Cursor != nil {
			input.Pagination.Cursor = &getBackupTransactions.UCGetBackupTransactionsListCursorBase{
	...
	var list getBackupTransactions.UCGetBackupTransactionsList
	err = s.dbApp.Exec(spanCtx, &list, func(d interface{}, in interface{}) (err error) {
		q, _ := d.(SQLDriverApp)

		var results *getBackupTransactions.UCGetBackupTransactionsList
		...
		if v, ok := in.(*getBackupTransactions.UCGetBackupTransactionsList); ok {
			v.List = results.List
