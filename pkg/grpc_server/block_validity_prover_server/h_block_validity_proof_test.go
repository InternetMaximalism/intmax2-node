...
func TestHandlerBlockValidityProof(t *testing.T) {
	const int3Key = 3
	...
	uc := mocks.NewMockUseCaseBlockValidityProverBlockValidityProof(ctrl)
	...
	cases := []struct {
		desc        string
		blockNumber string
		info        *blockValidityProverBlockValidityProof.UCBlockValidityProverBlockValidityProof
		prepare     func(*blockValidityProverBlockValidityProof.UCBlockValidityProverBlockValidityProof)
		success     bool
		message     string
		wantStatus  int
	}{
		...
		},
		{
			desc:        `UC error: fake`,
			blockNumber: "1",
			prepare: func(inputs *blockValidityProverBlockValidityProof.UCBlockValidityProverBlockValidityProof) {
				cmd.EXPECT().BlockValidityProverBlockValidityProof(gomock.Any(), gomock.Any(), gomock.Any()).Return(uc)
		...
		{
			desc:        `empty UC.ValidityProof result`,
			blockNumber: "1",
			info: &blockValidityProverBlockValidityProof.UCBlockValidityProverBlockValidityProof{
				ValidityPublicInputs: &block_validity_prover.ValidityPublicInputs{
					PublicState: &block_validity_prover.PublicState{
	...
	for i := range cases {
		t.Run(cases[i].desc, func(t *testing.T) {
			...
			r := httptest.NewRequest(
				http.MethodGet,
				fmt.Sprintf("http://"+gwServer.Addr+"/v1/block-validity-proof/%s", cases[i].blockNumber),
				http.NoBody,
			)
