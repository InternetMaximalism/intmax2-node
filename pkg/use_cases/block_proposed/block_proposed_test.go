...
func TestUseCaseBlockProposed(t *testing.T) {
	...
	txTreeLeaf := worker.TxTree{
		...
		TxHash:         txHash,
		LeafIndex:      leafIndex,
		TxTreeSiblings: txMerkleProof,
		TxTreeRootHash: &txTreeRoot,
		Signature:      "",
	}
	...
	for i := range cases {
		t.Run(cases[i].desc, func(t *testing.T) {
			...
			if cases[i].input != nil && cases[i].err == nil {
				assert.NotNil(t, resp)
				assert.Equal(t, resp.TxRoot, txTreeLeaf.TxTreeRootHash.String())
...
func sampleTxTree(t *testing.T) (*intMaxTree.TxTree, error) {
