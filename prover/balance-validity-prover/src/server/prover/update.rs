use crate::{
    ...
    proof::{generate_balance_update_proof_job, RedisResponse},
};
...
#[get("/proof/{public_key}/update/{request_id}")]
async fn get_proof(
    ...
    let proof_json = redis::Cmd::get(&get_balance_update_request_id(
        &public_key.to_hex(),
        &request_id,
    ))
...
async fn get_proofs(
    ...
    for request_id in &request_ids {
        let proof_json: Option<String> = redis::Cmd::get(&get_balance_update_request_id(
            &public_key.to_hex(),
            request_id,
        ))
        .query_async::<_, Option<String>>(&mut conn)
...
fn get_balance_update_request_id(public_key: &str, request_id: &str) -> String {
    format!("balance-validity/{}/update/{}", public_key, request_id)
}
