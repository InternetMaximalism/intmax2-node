use crate::{
    app::{
        interface::{DepositIndexQuery, ProofRequest, ProofResponse, ProofValue, ProofsResponse},
        state::AppState,
    },
...
async fn get_proof(
    ...
    let deposit_index = query_params.1
        .parse::<usize>()
        .map_err(error::ErrorInternalServerError)?;
    let proof = redis::Cmd::get(&get_receive_deposit_request_id(&public_key.to_hex(), deposit_index))
